...
with open(lime_struct_name, "r") as read_struct_file:
	...
	guards = {item['guards']:item for item in objs["guards"]}
	print(guards)
	for obj in objs: 
		...
		with open(lime_skeleton_asm, "r") as read_skeleton_file:
			...
			with open(lime_body_asm, "r") as read_body_file:
				...
				for method in obj["methods"]:
					m_name = obj["class_name"]+"_"+method+"_body"
					...
					print(dict(obj["guards"]))
					m_guard = obj["guards"][obj["class_name"]+method]
					skeleton = skeleton.replace(m_guard_name, m_guard)
				for action in obj["actions"]:
					a_name = obj["class_name"]+"_"+action+"_body"
					...
					a_guard = obj["guards"][obj["class_name"]+action]
