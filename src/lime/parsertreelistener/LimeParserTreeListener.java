...
public class LimeParserTreeListener  extends LimeGrammarBaseListener {
	...
	public void exitClassDecl(ClassDeclContext ctx) {
		System.out.println("exit class: definde methods: "+ ((ClassSymbol)currentScope).getNumberOfDefinedMethods()+ ((ClassSymbol)currentScope).getNumberOfFields()+((ClassSymbol)currentScope).getNumberOfDefinedActions());
		currentScope = currentScope.getEnclosingScope(); 
		System.out.println("global: " +this.symtab.GLOBALS);
		//System.out.println("methods: "+ ((ClassSymbol)this.symtab.GLOBALS.resolve("PQ")).getDefinedMethods());
		//System.out.println("actions: "+ ((ClassSymbol)this.symtab.GLOBALS.resolve("PQ")).getDefinedMethods());
		System.out.println("symbols: "+ this.symtab.GLOBALS.getSymbols() + this.symtab.GLOBALS.getNumberOfSymbols());
		className = "";
	}
	...
	public void exitGuardcompexpr(GuardcompexprContext ctx) {
		...
		String succeed = "_"+className+"_"+methodName+"_succeed";
		String output="";
		...
		switch(op) {
			case ">=": output += "JGE "+succeed+"\n";break;
			case "<=": output += "JLE "+succeed+"\n";break;
			case ">" : output += "JG "+succeed+"\n";break;
			case "<" : output += "JL "+succeed+"\n";break;
	...
	public void exitGuardeqexpr(GuardeqexprContext ctx) {
		...
		switch(op) {
			case "=": output += "JEQ "+succeed+"\n";break;
			case "!=": output += "JNE "+succeed+"\n";break;
			default: output += "failed\n";
		}
		regAvail.push(leftreg);
		regAvail.push(rightreg);
	...
	public void exitGuardandexpr(GuardandexprContext ctx) {
		...
		String succeed = "_"+className+"_"+methodName+"_succeed";
		String failed = "_"+className+"_"+methodName+"_checkguard_fail";
		...
		if(ctx.guardAtom(0)!=null && ctx.guardAtom(1)!=null) {
			...
			output += left;
			if(leftnot==1) output += "JE "+failed+"\n";
			else output += "JNE "+failed+"\n";
			output += right;
			if(rightnot==1)output += "JNE "+succeed+"\n";
			else  output += "JE "+succeed+"\n";
		}
		regAvail.push(leftreg);
		regAvail.push(rightreg);
		this.guardASMcode += output;
	...
	public void exitGuardorexpr(GuardorexprContext ctx) {
		...
		if(ctx.guardAtom(0)!=null && ctx.guardAtom(1)!=null) {
			...
			String left=String.format(strFormat, leftreg);
			output += left;
			if(leftnot==1) output += "JNE "+succeed+"\n";
			else output += "JE "+succeed+"\n";
			output += right;
			if(rightnot==1)output += "JNE "+succeed+"\n";
			else  output += "JE "+succeed+"\n";	
	...
	public void exitGuardatomexpr(GuardatomexprContext ctx) {
		...
		if(guardnot==1) output+="JNE "+succeed+"\n";
		else output+="JE "+succeed+"\n";
		this.guardASMcode+=output;
		regAvail.push(arg);
	}
